...
void Renderer::nanogui_init(GLFWwindow* window)
{
	...
	gui_1->addVariable("Catmull Curve on/off", m_bunny_animation->shown);

	gui_1->addGroup("Bunny Animation");

	
	gui_1->addVariable("Bunny is Moving", m_bunny_animation->isMoving);

	gui_1->addVariable("t1", m_bunny_animation->t1)->setSpinnable(true);
	gui_1->addVariable("t2", m_bunny_animation->t2)->setSpinnable(true);
	gui_1->addVariable("total moving time", m_bunny_animation->total_moving_time)->setSpinnable(true);
...
void Renderer::draw_scene(Shader& shader)
{
	...
	for (size_t i = 0; i < obj_list.size(); i++)
	{
		...
		if (obj_list[i].obj_name == "bunny")
		{
			...
			m_bunny_animation->update(delta_time);
