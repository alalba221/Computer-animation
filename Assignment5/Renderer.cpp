...
void Renderer::nanogui_init(GLFWwindow* window)
{
	...
	static auto X1 = gui_1->addVariable("X1", m_bone_animation->rotation_degree_vector[1][2]);
	static auto Y1 = gui_1->addVariable("Y1", m_bone_animation->rotation_degree_vector[1][0]);
	static auto Z1 = gui_1->addVariable("Z1", m_bone_animation->rotation_degree_vector[1][1]);

	static auto X2 = gui_1->addVariable("X2", m_bone_animation->rotation_degree_vector[2][2]);
	static auto Y2 = gui_1->addVariable("Y2", m_bone_animation->rotation_degree_vector[2][0]);
	static auto Z2 = gui_1->addVariable("Z2", m_bone_animation->rotation_degree_vector[2][1]);

	static auto X3 = gui_1->addVariable("X3", m_bone_animation->rotation_degree_vector[3][2]);
	static auto Y3 = gui_1->addVariable("Y3", m_bone_animation->rotation_degree_vector[3][0]);
	static auto Z3 = gui_1->addVariable("Z3", m_bone_animation->rotation_degree_vector[3][1]);


	gui_1->addButton("Reset Rotation", []() {
...
void Renderer::draw_bones(Shader& shader, Bone_Animation* m_bone_animation)
{
	...
	///Draw
	int i = 0;//color control
	for (vector<mat4>::iterator it = m_bone_animation->Matrixes.begin();
		it != m_bone_animation->Matrixes.end(); it++) {	
		glm::mat4 obj_mat = *it;
		//obj_mat = glm::translate(obj_mat, m_bone_animation->root_position);
		glUniformMatrix4fv(glGetUniformLocation(shader.program, "model"), 1, GL_FALSE, glm::value_ptr(obj_mat));
